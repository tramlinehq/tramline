...
describe Installations::Teamcity::Api do
  ...
  before do
    stub_request(:get, "#{server_url}/authenticationTest.html?csrf")
      .to_return(status: 200, body: "", headers: {"X-TC-CSRF-Token" => "test-csrf-token"})
  end
  ...
  describe "#server_version" do
    ...
    it "raises Teamcity::Error on client error" do
      stub_request(:get, "#{server_url}/app/rest/server")
    ...
    it "parses plain text error bodies" do
      stub_request(:get, "#{server_url}/app/rest/server")
  ...
  describe "#trigger_build" do
    ...
    it "raises Teamcity::Error with details on 4xx" do
      error_body = {"message" => "Could not find the given change 'abc123' in VCS roots of build configuration"}
      stub_request(:post, "#{server_url}/app/rest/buildQueue")
    ...
    it "raises Teamcity::Error on generic 404" do
      stub_request(:post, "#{server_url}/app/rest/buildQueue")
        .to_return(status: 404, body: {"message" => "Not found"}.to_json, headers: {"Content-Type" => "application/json"})
      ...
      }.to raise_error(Installations::Teamcity::Error)
    end
  end
  ...
  describe "#cancel_build" do
    ...
    it "silently handles 404 for non-existent builds" do
      stub_request(:get, "#{server_url}/app/rest/builds/id:999")
        .to_return(status: 404, body: {"message" => "Nothing is found by locator"}.to_json, headers: {"Content-Type" => "application/json"})

      expect { api.cancel_build(999) }.not_to raise_error
    end
  end
  ...
  describe "#find_build" do
    ...
    it "raises when build not found in results" do
      payload = {"build" => []}
      stub_request(:get, "#{server_url}/app/rest/builds")
        .with(query: hash_including("locator"))
        .to_return(status: 200, body: payload.to_json, headers: {"Content-Type" => "application/json"})
      ...
      }.to raise_error(Installations::Error) { |e|
        expect(e.reason).to eq(:workflow_run_not_found)
      }
    end

    it "translates 404 Teamcity::Error to workflow_run_not_found" do
      stub_request(:get, "#{server_url}/app/rest/builds")
        .with(query: hash_including("locator"))
        .to_return(status: 404, body: {"message" => "Nothing is found by locator"}.to_json, headers: {"Content-Type" => "application/json"})
