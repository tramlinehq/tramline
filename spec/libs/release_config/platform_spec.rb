...
describe ReleaseConfig::Platform do
  describe "#submissions?" do
    it "is true when submissions are present" do
      platform_config = ReleaseConfig::Platform::ReleaseStep.new({submissions: [{number: 1}]})
      expect(platform_config.submissions?).to be(true)
    end

    it "is false when submissions are not present" do
      expect(ReleaseConfig::Platform::ReleaseStep.new({submissions: nil}).submissions?).to be(false)
      expect(ReleaseConfig::Platform::ReleaseStep.new({submissions: [{}]}).submissions?).to be(false)
    end
  end
  ...
  describe "#submissions" do
    ...
    it "return first submission" do
      platform_config = ReleaseConfig::Platform::ReleaseStep.new({submissions: [{number: 1}, {number: 2}]})
      expect(platform_config.submissions.first.value).to match({number: 1})
    end

    it "returns last submission" do
      platform_config = ReleaseConfig::Platform::ReleaseStep.new({submissions: [{number: 1}, {number: 2}, {number: 3}]})
