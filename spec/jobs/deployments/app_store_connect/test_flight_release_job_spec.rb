...
describe Deployments::AppStoreConnect::TestFlightReleaseJob do
  describe "#perform" do
    context "when non app store deployment" do
      let(:play_store_deployment_run) { create_deployment_run_tree(:android, :started)[:deployment_run] }
    ...
    context "when app store deployment" do
      let(:app_store_deployment_run) { create_deployment_run_tree(:ios, :started)[:deployment_run] }
