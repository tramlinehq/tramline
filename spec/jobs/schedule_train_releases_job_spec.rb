...
describe ScheduleTrainReleasesJob do
  it "schedules release for active trains" do
    ...
    travel_to train.kickoff_datetime + 1.hour do
      ...
      expect(train.reload.scheduled_releases.last.scheduled_at).to eq(train.kickoff_datetime + train.repeat_duration)
    end
  end
  ...
  it "does not schedule release for active trains with nothing to schedule" do
    train = create(:train, :with_almost_trunk, :active, kickoff_at: Time.current + 7.hours, repeat_duration: 5.days)
    train.scheduled_releases.create!(scheduled_at: train.kickoff_datetime)
