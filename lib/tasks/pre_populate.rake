namespace :pre_populate do
  desc "Populates custom storage for a tenant"
  task :custom_storage, [:organization_id, :bucket, :bucket_region, :service_key] => :environment do |_, args|
    required = %i[organization_id bucket bucket_region service_key]
    if required.any? { |k| args[k].to_s.strip.empty? }
      abort "Usage: rake pre_populate:custom_storage[ORG_ID,BUCKET,REGION,SERVICE_KEY]"
    end
