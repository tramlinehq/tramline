module Installations
  ...
  class Teamcity::Api
    ...
    def list_build_configurations(project_id, transforms)
      locator = "affectedProject:(id:#{sanitize_locator_value(project_id)})"
      execute(:get, "/app/rest/buildTypes", params: {locator:})
        .then { |response| response&.dig("buildType") || [] }
        .then { |configs| configs.map { |c| c.merge("name" => "#{c["name"]} (#{c["projectName"]})") } }
    ...
    rescue Installations::Teamcity::Error => e
    ...
    def find_build(build_type_id, branch, commit_sha, transforms)
      ...
      raise Installations::Error.new("Could not find the build", reason: :workflow_run_not_found) if e.status_code == 404
      raise
    end
    ...
    def execute(verb, path, params: {}, json: nil)
      ...
      headers["X-TC-CSRF-Token"] = fetch_csrf_token if verb == :post
      ...
      if response.status.client_error?
        ...
        raise Installations::Teamcity::Error.new(response.status.code, body)
      end
    ...
    def fetch_csrf_token
      url = "#{server_url}/authenticationTest.html?csrf"
      response = HTTP.auth("Bearer #{access_token}")
        .headers(default_headers)
        .timeout(connect: 5, read: 5)
        .get(url)
      response.headers["X-TC-CSRF-Token"]
    rescue
      nil
    end
