module Installations
  class Jira::Api
    ...
    def projects(transforms = {})
      response = execute(:get, PROJECTS_URL.expand(cloud_id:).to_s)
      response["values"]
    end

    def project_statuses(project_key, transforms = nil)
      execute(:get, PROJECT_STATUSES_URL.expand(cloud_id:, project_key:).to_s)
    end

    def search_tickets_by_filters(project_key, release_filters, start_at: 0, max_results: 50)
