class Queries::Builds
  ...
  class << self
    def all(app:, limit:, offset:, sort_column:, sort_direction:)
      sort_column ||= DEFAULT_SORT_COLUMN
      sort_direction ||= DEFAULT_SORT_DIRECTION

      if app.android?
        android(app, limit, offset, sort_column, sort_direction)
      elsif app.ios?
        ios(app, limit, offset, sort_column, sort_direction)
      end
    end

    def count(app:)
      if app.android?
        base_android_query(app).size
      elsif app.ios?
        base_ios_query(app).size
      end
    end

    def android(app, limit, offset, sort_column, sort_direction)
      records =
        base_android_query(app)
    ...
    def ios(app, limit, offset, sort_column, sort_direction)
      records =
        base_ios_query(app)
