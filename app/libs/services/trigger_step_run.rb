module Services
  class TriggerStepRun
    ...
    def call
      ...
      status = if step.external_deployment?
        "finished"
      else
        "on_track"
      end
      step_run = release.step_runs.create!(step:, scheduled_at: Time.current, status:, commit:, build_version:, build_number:, sign_required:)
      step_run.automatons!
    end
