...
class Integration < ApplicationRecord
  ...
  class << self
    ...
    def existing_integrations_across_app(app, providable_type)
      Integration.connected.where(integrable_id: app.organization.apps.where.not(id: app.id), providable_type: providable_type).select("DISTINCT ON (providable_id, integrable_id) *")
    end
