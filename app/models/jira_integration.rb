...
class JiraIntegration < ApplicationRecord
  ...
  validate :validate_release_filters, if: -> { integrable.config.jira_config&.dig("release_filters").present? }
  ...
  def complete_access
    ...
    resources, tokens = API.get_accessible_resources(code, callback_url || redirect_uri)
    set_tokens(tokens)
  ...
  def fetch_tickets_for_release
    return [] if integrable.config.jira_config.blank?

    project_key = integrable.config.jira_config["selected_projects"]&.last
    release_filters = integrable.config.jira_config["release_filters"]
  ...
  def fetch_project_statuses(projects)
    ...
    with_api_retries do
      ...
      projects.each do |project|
        project_statuses = api.project_statuses(project["key"])
        statuses[project["key"]] = extract_unique_statuses(project_statuses)
      end
      statuses
    end
  rescue => e
  ...
  def validate_release_filters
    return if integrable.config.jira_config&.dig("release_filters").blank?

    integrable.config.jira_config["release_filters"].each do |filter|
