module Taggable
  ...
  def create_tag!(commitish, input_tag_name)
  ...
  def create_vcs_release!(commitish, release_diff, input_tag_name = base_tag_name)
    ...
    update!(tag_name: input_tag_name)
    event_stamp!(reason: :vcs_release_created, kind: :notice, data: {provider: vcs_provider.display, tag: input_tag_name})
  rescue Installations::Error => ex
    raise unless [:tag_reference_already_exists, :tagged_release_already_exists].include?(ex.reason)
    create_vcs_release!(commitish, unique_tag_name(input_tag_name, commitish), release_diff)
  end
