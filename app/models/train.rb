...
#  allow_upcoming_release_submissions             :boolean          default(FALSE), not null
...
class Train < ApplicationRecord
  ...
  has_many :lifecycle_hooks, inverse_of: :train, dependent: :destroy
  ...
  def valid_schedule
    if release_schedule_changed?
      ...
      errors.add(:kickoff_at, "the schedule kickoff should be in the future") if kickoff_at_changed? && kickoff_at && kickoff_at_app_time <= Time.current
